...
public class Favorite {
	...
	public Favorite(int userId) {
		this.userId = userId;
		this.favoriteMails = new ArrayList<>();
	}
...
//        List<ZMail> saved = new ArrayList<>();
		String sql = """
				    SELECT z.*
				    FROM zmail z
				    JOIN favorite_mails s ON z.id = s.mailId
				    WHERE s.userId = ?;
				""";
...
//        return saved;
	...
	public void addMail(ZMail mail) {
		...
		String sql = "INSERT INTO favorite_mails (userId, mailId) VALUES (?, ?)";
...
//		}
	...
	int showMails() {
		...
		return favoriteMails.size();
	}
	
	
	public void removeFavoriteMail(int userId, int mailId) {
	    String sql = "DELETE FROM favorite_mails WHERE userId = ? AND mailId = ?";
